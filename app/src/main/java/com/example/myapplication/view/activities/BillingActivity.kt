...
class BillingActivity : DcbBaseActivity() {
    ...
    private fun updateUI(mBillings: BillingResp) {
        try {
            tvBillingDate.text = mBillings.wallet_details?.updated_at
            tvBillingAmount.text = "₹ ${mBillings.wallet_details?.current_billing_amount}"
            mAdapter.addList(mBillings.wallet_details?.transactions!!)
        } catch (e: Exception) {
            e.printStackTrace()
            showErrorDialog(3, getString(R.string.something_went_wrong))
        }
    }
    ...
    private fun showErrorDialog(type: Int?, msg: String?) {
        ...
        if (type == 1) {
            ...
            animationJson = "something_went_wrong.json"
        } else {
            //something else
            animationJson = "something_went_wrong.json"
        }
